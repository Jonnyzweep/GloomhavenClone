...
#define HEXNORTHEAST 5
	...
	private static boolean isPointyTop = true;
...
*********************************************************/
	public static Polygon hex (int x, int y) {
		double[] centres = calculatePixelHexCentre(x, y, r);
		int[][] edges = calculateHexEdges((int) centres[0], (int) centres[1],r);



		return new Polygon(edges[0], edges[1],6);
...
*****************************************************************************/
	...
	{
		double ht  = r;
		double w = Math.sqrt(3) / 2 * r;
		char c='o';
		int x = (int) (w + i * w);
		int y = (int) (ht + j * ((3.0 / 4.0) * ht));
		if (j % 2 == 1) { // this checks for odd/even: if odd, offset 2nd, 4th,...
			x = (int) (x + (w / 2));
		}
		if (n < 0) {
...
*****************************************************************************/
	public static Point pxtoHex(int mx, int my)
	{
		...
		int x = (int) (mx / w); //this gives a quick value for x. It works only on odd cols and doesn't handle the triangle sections. It assumes that the hexagon is a rectangle with width s+t (=1.5*s).
		int y = (int) ((my /((3.0*4.0) / ht))); //this gives the row easily. It needs to be offset by h/2 (=r)if it is in an even column
	 ...
	 */
	public static int[][] calculateHexEdges(int centrex, int centrey, int size) {
	 ...
	 */
	public static double[] calculatePixelHexCentre(int ax, int ay, int size) {
		double h;
		double w;
